<template>
    <LoadingComponent :props="loading"/>
    <div class="w-full max-w-3xl mx-auto rounded-2xl flex overflow-hidden gap-y-6 bg-white shadow-card mb-24 !sm:mb-0">
        <img src="images/required/auth.jpg" class="w-full hidden sm:block sm:max-w-xs md:max-w-sm flex-shrink-0" loading="lazy" alt="auth">

        <form class="w-full p-6" @submit.prevent="submitRegister">
            <div class="text-center mb-8">
                <h2 class="capitalize text-2xl mb-2 font-bold text-primary">Register</h2>
            </div>

            <div class="mb-6">
                <label for="formName" class="capitalize text-sm mb-1 field-title font-medium required">name</label>
                <input v-model="form.name" :class="errors.name ? 'border-red-500' : ''" id="formName" type="text"
                    class="w-full h-12 px-4 rounded-lg text-base border border-[#D9DBE9] hover:border-sky-400 focus-within:border-sky-400 transition-all duration-500">
                <small class="text-red-500" v-if="errors.name">{{ errors.name[0] }}</small>
            </div>

            <div class="mb-6">
                <label for="formEmail" class="capitalize text-sm mb-1 field-title font-medium required">email</label>
                <input v-model="form.email" :class="errors.email ? 'border-red-500' : ''" id="formEmail" type="text"
                    class="w-full h-12 px-4 rounded-lg text-base border border-[#D9DBE9] hover:border-sky-400 focus-within:border-sky-400 transition-all duration-500">
                <small class="text-red-500" v-if="errors.email">{{ errors.email[0] }}</small>
            </div>

            <div class="mb-6">
                <label for="formPassword" class="capitalize text-sm mb-1 field-title font-medium required">password</label>
                <input v-model="form.password" :class="errors.password ? 'border-red-500' : ''" id="formPassword" type="password"
                    class="w-full h-12 px-4 rounded-lg text-base border border-[#D9DBE9] hover:border-sky-400 focus-within:border-sky-400 transition-all duration-500">
                <small class="text-red-500" v-if="errors.password">{{ errors.password[0] }}</small>
            </div>

            <button type="submit"
                class="font-bold text-center w-full h-12 leading-12 rounded-full bg-sky-400 text-white capitalize mb-6">
                register
            </button>

            <div class="flex items-center justify-center gap-1.5">
                <span class="font-medium text-text">Already have an account?</span>
                <router-link class="capitalize font-bold text-sky-400" :to="{ name: 'frontend.login' }">
                    login
                </router-link>
            </div>
        </form>
    </div>
</template>

<script>
import LoadingComponent from '../../admin/components/LoadingComponent.vue';
import { useToast } from "vue-toastification";

export default {
    name: 'Register',
    components: {
        LoadingComponent
    },
    setup(){
        const toast = useToast(); // Initialize toast
        return { toast };
    },
    data() {
        return {
            loading: {
                isActive: false
            },
            form: {
                name: "",
                email: "",
                password: ""
            },
            errors: {}
        }
    },
    methods: {
        async submitRegister() {
            try {
                this.loading.isActive = true;
                this.errors = {};

                // Dispatch the 'register' action from the 'auth' module
                const response = await this.$store.dispatch('auth/register', this.form);

                // If your backend requires OTP verification
                this.$router.push({ name: 'frontend.verifyOtp', query: { email: this.form.email } });

                this.toast("Registration successful! Please check your email for the OTP.");

            } catch (error) {
                // Handle Laravel validation errors (Status 422)
                if (error.response && error.response.status === 422) {
                    this.errors = error.response.data.errors;
                } else {
                   this.toast("Registration failed. Please try again.");
                }
            } finally {
                this.loading.isActive = false;
            }
        }
    }
}
</script>
