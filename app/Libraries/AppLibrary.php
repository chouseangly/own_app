    ...
    public static function date($date, $pattern = null): string
{
    ...
    public static function numericToAssociativeArrayBuilder($array): array
    {
        ...
        // First Pass: Create a lookup table of all items and initialize children
        foreach ($array as $arr) {
            $arr['children'] = [];
            $lookup[$arr['id']] = $arr;
        }

        // Second Pass: Link children to parents or add to top-level
        foreach ($lookup as $id => &$item) {
            if ($item['parent'] == 0) {
                // It's a top-level menu
                $buildArray[] = &$item;
            } else {
                // It's a child; attach it to its parent if the parent exists
        ...
        // Filter out empty parent menus (like "Reports" if it has no children)
        foreach ($buildArray as $key => $build) {
